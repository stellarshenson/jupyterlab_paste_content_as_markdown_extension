...
- **Rich text support** - Handles content copied from Word, Google Docs, Confluence, Notion, and other rich text editors that place HTML on the clipboard. These carry bold, italic and strikethrough as CSS on a plain `<span>` rather than as tags, and Word writes its bulleted and numbered lists as ordinary paragraphs with the bullet as literal text; both are rebuilt into markdown, lists keeping their nesting and the number they started at
